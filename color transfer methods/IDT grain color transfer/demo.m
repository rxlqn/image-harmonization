...
%numForEachCat={70,100,40,5};
numForEachCat={95,130,50,8};
...
for TypeIndex=3:4
    ...
    for catId=1:80
        ...
        %     fprintf('%d reminder: %d', length(reminder),n-1);
        if n-1==1
            continue;
        end
        %fprintf('reminder: %d\n', n-1);
        ...
        for iterate=1:numForEachCat{TypeIndex}
            %         imgId1 = 297353; %has several segmentation parts of a person
            %         imgId2 = 261318; %has several segmentation parts of a person
            %         imgId1 = 137281;
            %         imgId2 = 75052; %640*444 uint8, a gray imag
%             imgId1 = 205289;
%             imgId2 = 220732;
...
%             subplot(1,2,2), imshow(I2), title('reference');
            ...
            filepath=pwd;
            cd(sprintf('E:\\coco\\composite_images\\%s_idt\\groundtruth',dataType));
            imwrite(I1,sprintf('%s_%g_idt_truth.jpg',dataType,imgId1));
            cd(filepath);
            
            %load annotations for target image
            %select the largest segmented object
...
%             subplot(1,2,1),coco.showAnns(anns1);
            ...
            filepath=pwd;
            cd(sprintf('E:\\coco\\composite_images\\%s_idt\\mask',dataType));
            imwrite(bm1,sprintf('%s_%g_idt_mask.jpg',dataType,imgId1));
            cd(filepath);
